...
export default function PsychTestFeedback({ messageContent, allMessages, sessionConsent, onConsentGiven }) {
  ...
  const handleSubmitAndSave = async () => {
    ...
    try {
      ...
      await Promise.all([
        base44.entities.PsychTestFeedback.create({
          user_id: user.id,
        ...
        base44.entities.PsychTestLog.create({
      ...
      setDone(true);
      toast.success('Feedback and session saved. Thank you for helping train Luna! 💚');
    } catch (err) {
      console.error(err);
  ...
  if (done) {
    return (
      <div className="mt-3 p-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-2xl flex items-center gap-2 text-xs text-emerald-700 dark:text-emerald-300 font-medium">
        ...
        Feedback and session saved — thank you for helping train Luna! 💚
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 space-y-2">
      <div className="p-2.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-700 rounded-xl flex items-start gap-2 text-[11px] text-amber-800 dark:text-amber-300 leading-snug">
        ...
        <span>
          <strong>Test Mode:</strong> Clinician feedback mode. Data is saved for AI improvement only — <strong>not for clinical decisions</strong>.
        </span>
      </div>
      ...
      <div className="p-3 bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800 rounded-2xl space-y-3">
        ...
        <label className="flex items-start gap-2 cursor-pointer">
          <input
            ...
            onChange={(e) => { if (e.target.checked && onConsentGiven) onConsentGiven(); }}
        ...
        <Button
          ...
          disabled={saving || !allRated || !sessionConsent}
          className="w-full rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-xs h-8 gap-1"
        >
